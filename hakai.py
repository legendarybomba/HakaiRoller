...
# Static Difficulty Reference Matrix
...
if is_double_feature:
    ...
    print("[2] Modulated Focus (Heated)", flush=True)
    print("[3] Sweat Session (Hard)", flush=True)
    while True:
        intensity_choice = input("\nSelect difficulty (1-3): ").strip()
        if intensity_choice in ["1", "2", "3"]:
            break
        print("⚠️  Invalid choice. Use 1, 2, or 3.", flush=True)
    
    if intensity_choice == "1": single_diff = "Easy"
    elif intensity_choice == "2": single_diff = "Heated"
    else: single_diff = "Hard"
...
game_session_cards = {}  # Map: game_name -> list of card_data dictionaries
...
while True:
    ...
    while rolls_queue:
        ...
        # --- FIX #2 & #3: LIVE MID-SPIN STACKING WITH SHIFTING LOGIC ---
        while len(active_deck) < 3:
            while True:
                stack_choice = input(f"\n🔥 Stack card {len(active_deck)+1} into matrix? (y/n): ").strip().lower()
                if stack_choice in ['y', 'n']:
                    break
                print("⚠️  Use 'y' or 'n'.", flush=True)
                
            if stack_choice == 'y':
                # Shift logic calculation based on the baseline anchor difficulty
                anchor_idx = DIFF_LEVELS.index(target_difficulty)
                # Random choice: -1 (one tier lower), 0 (same tier), 1 (one tier higher)
                shift = random.choice([-1, 0, 1])
                new_idx = max(0, min(2, anchor_idx + shift))
                stacked_diff = DIFF_LEVELS[new_idx]
                
                print(f"🔮 Fetching a {stacked_diff} stacked contract...", flush=True)
                stacked_card_data = oracle.generate_challenge(game=winner, difficulty=stacked_diff)
                
                if stacked_card_data["status"] == "error":
                    print("❌ Stacking failed. Pipeline blocked.", flush=True)
                    break
                    
                if is_sudden_death:
                    stacked_card_data["base_payout"] = 25
                    
                active_deck.append(stacked_card_data)
                print("--------------------------------", flush=True)
                display_wrapped_objective(f"⚡ Obj {len(active_deck)} Stacked ({stacked_diff}): ", stacked_card_data["objective"])
                print(f"💰 Payout: {stacked_card_data['base_payout']} XP/G", flush=True)
            else:
                break
    ...
    for game in current_review_games:
        ...
        deck = game_session_cards[game]
        
        for idx, card in enumerate(deck, 1):
            total_attempted_cards += 1
            while True:
                print(f"\n   👉 Clear Challenge {idx} ({card['difficulty']})?", flush=True)
    ...
    if overtime == "+":
        ...
        if success_rate == 1.0:
            ...
            is_sudden_death = False
            # Recalibrate using our new middle-tier spectrum map
            if success_rate >= 0.75: bonus_diff = "Hard"
            elif success_rate >= 0.40: bonus_diff = "Heated"
            else: bonus_diff = "Easy"
